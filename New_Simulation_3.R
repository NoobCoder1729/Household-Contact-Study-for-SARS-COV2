...
household.vec=c(5,10,20,30,40,50,60,70,80)

#p.val.mse.fn.new.1(1450,2,.05,.1,90,.999)

start.time=Sys.time()

df1=data.frame(t(matrix(unlist(lapply(household.vec, p.val.mse.fn.new.1,ind.ct=1368,p=2,prob.trt=.05,prob.contr=.1,rho1=.999)),nrow=4,ncol=length(household.vec))))
...
df1=p.val.mse.fn.q2(1368,2,prob.trt=.1,prob.contr=.1,one.percent=60,rho1=.999)
...
legend(0, 1, legend=c(" GEE", "Logistic"),
       col=c("blue","red"), lty=1:1, cex=0.67)
